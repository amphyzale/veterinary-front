...
import { LoginResponse, UserLoginInfo, UserRegisterInfo } from '../models/authentication.model';
...
export class AuthenticationService {
  ...
  login(loginInfo: UserLoginInfo): Observable<LoginResponse> {
    ...
    return this.http.post<LoginResponse>(this.apiHelper.getUrl(METHOD_LOGIN), body, this.httpOption)
      .pipe(tap(res => {
        this.tokenStorageService.saveToken(res.token);
        this.tokenStorageService.saveUser(res.username);
      }));
  }
  ...
  register(registerInfo: UserRegisterInfo): Observable<void> {
    ...
    return this.http.post<any>(this.apiHelper.getUrl(METHOD_REGISTER), body, this.httpOption);
  }
}
